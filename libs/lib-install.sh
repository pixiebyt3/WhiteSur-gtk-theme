...
install_waterfox_theme() {
  local TARGET_DIR="${WATERFOX_THEME_DIR}"
...
config_waterfox() {
  local TARGET_DIR="${WATERFOX_THEME_DIR}"
  local WATERFOX_DIR="${WATERFOX_DIR_HOME}"

  killall "waterfox" &> /dev/null || true

  for d in "${WATERFOX_DIR}/"*"default"*; do
    rm -rf                                                                                  "${d}/chrome"
    udo ln -sf "${TARGET_DIR}"                                                              "${d}/chrome"
    rm -rf                                                                                  "${d}/user.js"
    udoify_file                                                                             "${d}/user.js"
...
edit_waterfox_theme_prefs() {
  local TARGET_DIR="${WATERFOX_THEME_DIR}"

  [[ ! -d "${TARGET_DIR}" ]] && install_waterfox_theme ; config_waterfox
  udo ${EDITOR:-nano}                                                                         "${TARGET_DIR}/userChrome.css"
  udo ${EDITOR:-nano}                                                                         "${TARGET_DIR}/customChrome.css"
}
