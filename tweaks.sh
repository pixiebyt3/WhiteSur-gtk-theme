...
usage() {
  ...
  helpify "-e, --edit-waterfox"  "[(monterey|flat)|alt|(darker|adaptive)]"           "  Edit '${THEME_NAME}' theme for Waterfox settings and also connect the theme to the current Firefox profiles" ""

  helpify "" "" "Tweaks for waterfox" "options"
  sec_title "-f, --waterfox" "      [(monterey|flat)|alt|(darker|adaptive)]"       "  Without options default WhiteSur theme will install..."                      "  Options:"
  sec_helpify "1. monterey" "      [3+3|3+4|3+5|4+3|4+4|4+5|5+3|5+4|5+5]"           "  Topbar buttons number: 'a+b'"                                                "  a: left side buttons number, b: right side buttons number"
  sec_helpify "2. flat" "          Monterey alt version"                            ""                                                                              "  Flat round tabs..."
  sec_helpify "3. alt" "           Alt windows button version"                      ""                                                                              "  Alt windows button style like gtk theme"
  sec_helpify "4. darker" "        Darker Firefox theme version"                    ""                                                                              "  Darker Firefox theme version"
  ...
  helpify "-e, --edit-waterfox"  "[(monterey|flat)|alt|(darker|adaptive)]"           "  Edit '${THEME_NAME}' theme for Waterfox settings and also connect the theme to the current Waterfox profiles" ""
...
while [[ $# -gt 0 ]]; do
  ...
  case "${1}" in
    ...
    -f|--firefox|-e|--edit-firefox|--waterfox|--edit-waterfox)
      case "${1}" in
        -f|--firefox)
          firefox="true" ;;
        -e|--edit-firefox)
      ...
      if ! has_command waterfox && ! has_command firefox && ! has_command firefox-bin && ! has_flatpak_app org.mozilla.firefox && ! has_snap_app firefox && ! has_command firefox-developer-edition; then
        prompt -e "'${1}' ERROR: There's no Firefox/Waterfox installed in your system"
        has_any_error="true"
      elif [[ ! -d "${FIREFOX_DIR_HOME}" && ! -d "${FIREFOX_FLATPAK_DIR_HOME}" && ! -d "${FIREFOX_SNAP_DIR_HOME}" ]]; then
        prompt -e "'${1}' ERROR: Firefox/Waterfox is installed but not yet initialized."
        prompt -w "'${1}': Don't forget to close it after you run/initialize it"
        prompt -w "'${1}': In case you forgot - if using Waterfox please run ./fix.sh"
        has_any_error="true"
      elif pidof "waterfox" &> /dev/null || pidof "firefox" &> /dev/null || pidof "firefox-bin" &> /dev/null; then
        prompt -e "'${1}' ERROR: Firefox/Waterfox is running, please close it"
        has_any_error="true"
      fi; shift ;;
    -g|--gdm)
...
#  fi
  ...
  if [[ "${waterfox}" == 'true' || "${firefox}" == 'true' && "${gdm}" != 'true' ]]; then
    prompt -i "Removing '${firefoxtheme}' Firefox/Waterfox theme... \n"
    remove_firefox_theme
    prompt -s "Done! '${firefoxtheme}' Firefox/Waterfox theme has been removed. \n"
  fi
else
...
#  fi
  ...
  if [[ "${waterfox}" == 'true' || "${firefox}" == 'true' || "${edit_firefox}" == 'true' || "${edit_waterfox}" == 'true' ]]; then
    if [[ "${darker}" == '-darker' && "${adaptive}" == '-adaptive' ]]; then
      prompt -w "FIREFOX/WATERFOX: You can't use 'adaptive' and 'darker' at the same time. \n"
      prompt -i "FIREFOX/WATERFOX: Setting to adaptive only... \n"
      darker=''
    fi

    if [[ "${waterfox}" == 'true' && "${firefox}" == 'true' && "${gdm}" != 'true' ]]; then
      prompt -i "Installing '${firefoxtheme}' Firefox/Waterfox theme... \n"
      install_firefox_theme
      prompt -s "Done! '${firefoxtheme}' Firefox/Waterfox theme has been installed. \n"
    fi

    if [[ "${edit_waterfox}" == 'true' && "${edit_firefox}" == 'true' && "${gdm}" != 'true' ]]; then
      prompt -i "Editing '${firefoxtheme}' Firefox/Waterfox theme preferences... \n"
      edit_firefox_theme_prefs
      prompt -s "Done! '${firefoxtheme}' Firefox/Waterfox theme preferences has been edited. \n"
    fi

    if [[ "${gdm}" == "false" ]]; then
      prompt -w "FIREFOX/WATERFOX: Please go to [Firefox/Waterfox menu] > [Customize...], and customize your Firefox/Waterfox to make it work. Move your 'new tab' button to the titlebar instead of tab-switcher. \n"
      prompt -i "FIREFOX/WATERFOX: Anyway, you can also edit 'userChrome.css' and 'customChrome.css' later in your Firefox/Waterfox profile directory. \n"
    fi
  fi
fi
